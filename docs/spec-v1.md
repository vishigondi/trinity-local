...
### MCP tool surface (11 total: 6 canonical + 3 v1.5 + 2 launch-arc)

The original spec wanted 3 tools (council / query_lens / add_pair). Current ships 11 —
6 canonical (v1.0), 3 v1.5 additions, 2 launch-arc additions (`handoff` from tick #119,
`get_eval_summary` from post-#122).
...
3. **11 MCP tools shipped (6 canonical + 3 v1.5 additions + 2 launch-arc).** The 3-tool subset breaks `record_outcome` (supervision loop persistence), `get_council_status` (async polling), `get_persona` (lens.md hand-off). v1.0 ships 6: `route`, `run_council`, `record_outcome`, `search_prompts`, `get_persona`, `get_council_status`. v1.5 adds `ask`, `get_picks`, `mark_pick_wrong`. Launch-arc adds `handoff` (tick #119 — cross-provider conversation continuity, killer-hook mechanism for the 60-second demo; powers tasks #115/#120/#121) and `get_eval_summary` (post-#122 — latest empirical-benchmark result for the agent surface).
