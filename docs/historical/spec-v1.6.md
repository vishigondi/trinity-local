...
The MCP tool surface stays at 8 (no new MCP tools needed for v1.6 — `search_prompts` retired 2026-05-17, `get_eval_summary` retired 2026-05-18, `record_outcome` retired 2026-05-21, and `handoff` retired 2026-05-26 dropped it from 11 → 8 over the launch arc; `import_provider_memory` shipped in the same window to backfill the in-protocol provider loop slot).
