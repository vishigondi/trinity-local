...
- `437e93a` — py_file_count stale at 113, actually 119
- `9458695` — tool-count drift + duplicate paragraph
- `6cf57de` — claude.md count fix
- `d7ea967` — de-stale claude.md L13 status line
- `67910a4` — test_count 1652→1653 across 10 surfaces
- `6483768` — claude.md "21-command surface" stale → canonicalize
...
- `7d561a8` — orphan retired-CLI handler stubs (`commands/distill.py`
  + `bootstrap_pairs.py`) — exactly the module-orphan case
  `find_orphans.py` now catches at PR time.
- `d5d6415` — orphan `canUseShortcut()` getter (function-level;
  vulture territory, but same family)
- `f87d199` — dead `watcher` subdir in a test fixture

**Conclusion:** #188's reachability finder would have flagged
`7d561a8` directly. The moves substrate (this session, -4,400 LOC)
is the same class at 50× the scale. Validated.

### Stale retired-name references — ≥4 hits, validated #189

- `26f5a7b` — CONTRIBUTING referenced retired `dispatch_runner`
- `742207a` — cross-platform-spec stale rating-UX refs ("escaped #134")
- `920b2d5` — browser-extension README: 4 stale claims
- `49d9344` — `watcher_dir()` source-comment-vs-registry drift

**Conclusion:** #189's AST guards catch the *code* references
(`from trinity_local.<retired>`). The *doc* references (`742207a`,
`920b2d5`) need a doc-side term guard — that's the still-open leg of
#189's original scope. The "escaped #134" note in `742207a` is
...
- `030bad4` — "memory-compare: fix two bugs that made Mode-1
...
- `b3882ae` — extension_repair counted sentinel/orphan files as
  captures (wrong non-zero output; adjacent class).

**Conclusion:** 2 clean hits (030bad4 + moves) + 1 adjacent. The
...
- `e492e3c` — "ci: fix ... optimistic MLX probe" — the probe
...
1. **Doc-side retired-name guard** (extends #189): scan `class: live`
   ...
   Would have caught `742207a` + `920b2d5`. ≥2 hits, low effort —
