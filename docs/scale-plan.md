...
> decision record).
>
> **MCP surface (v1.0 canonical 6 + v1.5 `ask` + `get_cortex_rules`, 8 total):**
...
> returns `escalate_hint=compare` when trust is low) and `get_cortex_rules`
> (agent-facing introspection into extracted routing patterns). Hot-path
