...
**Three-tier framing** (architecture ratified by a launch council, 2026-05-16):
...
**T-0 engineering sequence (run in order before any tweet ships):**
  ...
  # Or run individually:
  #   pytest -q                                  # ~<!-- canonical:test_count -->2445<!-- /canonical --> tests
  #   pytest tests/test_doc_count_consistency.py # <!-- canonical:doc_consistency_guards -->111<!-- /canonical --> doc-consistency guards
  ...
  # 3. Tag whatever pyproject.toml currently has (v1.7.4 was tagged
  #    when v1.7 shipped May 13–15, 2026; v1.7.5 is the post-launch
  #    cleanup described in CHANGELOG.md). If pyproject moved forward
  #    since the last tag, cut a fresh tag at the current version:
  ...
  curl -sf -o /dev/null -w "%{http_code}\n" <!-- canonical:github_repo_url -->https://github.com/vishigondi/trinity-local<!-- /canonical --> # → 200
