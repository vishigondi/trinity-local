...
N is now matched across providers — that's what shipped end-to-end as of 2026-05-23. The harness itself is the differentiator: `eval-build` mines `(prompt, rejected_response, rejection_type)` triples from your actual transcripts via the Stage 0 turn-pair gap extractor (REFRAME / COMPRESSION / REDIRECT / SHARPENING, with validators in `me/turn_pairs.py` — e.g. COMPRESSION requires user_text ≤ model_text/10). `eval-run --target <provider>` scores any model against YOUR empirical rejections using `lens.md` as judge rubric. Freshest run on Vishi's corpus (2026-05-23) — all three providers on the SAME N=45 suite: claude 0.79 aggregate (SHARPENING 0.82 / REFRAME 0.81 / REDIRECT 0.80 / COMPRESSION 0.48), codex 0.76 (SHARPENING peak 0.86 — beats claude there — and COMPRESSION 0.78 also beats claude), antigravity 0.61 (clear COMPRESSION weakness at 0.08). The per-axis split is the actual signal; aggregate hides where each provider wins. No provider can build this benchmark — they can't see cross-provider rejection signal. N grows with corpus.
