...
Agreed it's thin today — that's what shipped end-to-end, not the depth claim. The harness itself is the differentiator: `eval-build` mines `(prompt, rejected_response, rejection_type)` triples from your actual transcripts via the Stage 0 turn-pair gap extractor (REFRAME / COMPRESSION / REDIRECT / SHARPENING, with validators in `me/turn_pairs.py` — e.g. COMPRESSION requires user_text ≤ model_text/10). `eval-run --target <provider>` scores any model against YOUR empirical rejections using `lens.md` as judge rubric. First real run on a 3-provider lineup: claude 1.00 / codex 0.80 / gemini 0.57 (logged 2026-05-14). The point isn't current N; it's that no provider can build this benchmark — they can't see cross-provider rejection signal. N grows with corpus.
