...
`src/trinity_local/mcp_server.py` exposes these tools to any MCP-compatible harness (Claude Code, Codex CLI, Gemini CLI, Cursor). v1.0 ships 5; v1.5 adds three — `ask` for cheap single-call routing, `get_picks` for agent-facing introspection, `mark_pick_wrong` for the harness-callable user veto; launch-arc adds two — `handoff` (cross-provider conversation continuity) and `get_eval_summary` (latest empirical-benchmark result) — for 10 total:
