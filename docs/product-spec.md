...
11. **Test suite: 519 passing.** Cortex consolidation (incl. end-to-end centroid integration with real TF-IDF embeddings + geometric median + coherence + bimodality + bimodal cortex fall-through), `ask` orchestration + MCP handler, tool-triggered incremental ingest (10 tests for cursor persistence + dedupe + deadline-respect), HF offline pinning (default + user override), plus the v1.0 base.
