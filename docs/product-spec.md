...
### MCP — v1.0 canonical 6 + v1.5 trio + launch-arc `handoff` + `get_eval_summary`

`src/trinity_local/mcp_server.py` exposes these tools to any MCP-compatible harness (Claude Code, Codex CLI, Gemini CLI, Cursor). v1.0 ships 6; v1.5 adds three — `ask` for cheap single-call routing, `get_picks` for agent-facing introspection, `mark_pick_wrong` for the harness-callable user veto; launch-arc adds two — `handoff` (cross-provider conversation continuity) and `get_eval_summary` (latest empirical-benchmark result) — for 11 total:
...
7. **MCP tool surface (v1.0 canonical 6 + v1.5 trio + launch-arc `handoff` + `get_eval_summary`).** v1.0: `route`, `run_council` (subsumes `judge`), `record_outcome`, `search_prompts`, `get_persona`, `get_council_status`. v1.5: `ask` (cheap single-call default), `get_picks` (agent-facing introspection), `mark_pick_wrong` (harness-callable user veto). Launch-arc: `handoff` (cross-provider conversation continuity) and `get_eval_summary` (latest empirical-benchmark result). 11 total.
