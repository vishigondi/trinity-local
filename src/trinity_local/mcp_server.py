...
def _resource_catalog() -> list[tuple[str, str, str, str, Any]]:
    ...
    AGENTS.md is listed too — once #163 lands, it's the
    lens-derived guidance any non-MCP harness reads natively.
    Cold-install case: the file doesn't exist yet; read_resource
    returns a stub "AGENTS.md not yet generated — run trinity-local
    dream" rather than 404, so the agent gets actionable feedback.
    """
    from . import state_paths as _sp
    home = _sp.trinity_home()
    ...
    return [
        ...
        (
            "trinity://AGENTS.md",
            "Trinity Lens-Derived Agent Guidance",
            "Lens + core memory rendered as AGENTS.md — the standard format Codex CLI, Cursor, and Cline read natively. Regenerated by `trinity-local dream`.",
            "text/markdown",
            lambda: home / "AGENTS.md",
        ),
        (
