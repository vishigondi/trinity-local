...
# `adapters/gemini.js`. Unlike claude.ai / chatgpt.com (whose canonical
# state endpoints return the full message tree), Google's `batchexecute`
# RPC only emits the assistant's reply to the LAST user turn. The adapter
# best-effort extracts `assistant_text` + `conv_id`; the raw body is
# preserved in `_raw_body` for re-extraction if Google's frame shape
# moves.
#
# Wire shape (what adapter writes per gemini.js):
#   {
#     "provider": "gemini",
#     "kind": "adapter_stream",
#     "conv_id": "<from URL>",
#     "message_id": "<best-effort>" | null,
#     "url": "<batchexecute URL>",
#     "captured_at": "<ISO8601>",
#     "frames_count": <int>,
#     "events_count": <int>,
#     "assistant_text": "<best-effort prose>",
#     "_raw_body": "<full body text>"
#   }
#
# We only have the assistant turn (Google's RPC doesn't echo the user
# prompt back). The parser still produces a SessionRecord — downstream
# `iter_prompt_turns` will yield the assistant message as part of a
# turn window so the corpus picks up Gemini's responses for lens
# building. The user prompt is recovered from claude.ai / chatgpt
# adapters; here we're closing the gemini-orphan-files gap.
...
def parse_captured_gemini_conversation(path: Path) -> SessionRecord | None:
    ...
    Unlike the claude.ai / chatgpt.com parsers (which read canonical
    full-conversation state), this parser only produces ONE assistant
    message per file because Gemini's batchexecute RPC is reply-only.
    """
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
    ...
    messages: list[SessionMessage] = []
    # User turn — only present when adapter could extract it from the
    # batchexecute REQUEST body (page-hook.js snapshots init.body for
    # gemini.google.com fetches). Without this, the capture contributes
    # zero PromptTurn entries because iter_prompt_turns only yields
    # user-facing turns. Older captures (pre-v1.8) won't have this
    # field; the assistant turn alone is still recorded as context.
    if user_text and user_text.strip():
        messages.append(SessionMessage(
            role="user",
            text=user_text.strip(),
    ...
    return SessionRecord(
        ...
        cwd=None,
        project_hint=None,
        # First ~80 chars of the assistant text serve as a stand-in title —
        # the batchexecute RPC doesn't carry conversation title.
        title=(assistant_text.strip()[:80] or None),
        model=None,
        cli_name="gemini_webapp",
        cli_version=None,
        source_format="gemini_browser_capture",
