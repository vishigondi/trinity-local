...
This wires the Trinity MCP tools into every harness the user has installed. v1.0 ships 6 tools (`run_council`, `route`, `record_outcome`, `search_prompts`, `get_persona`, `get_council_status`); v1.5 adds `ask` (cheap default single-call routing) and `get_cortex_rules` (introspection into extracted routing patterns) — 8 total. The install edits `~/.claude.json`, `~/.gemini.json`, and `~/.codex/config.toml` — non-destructive, only adds the Trinity entry.
