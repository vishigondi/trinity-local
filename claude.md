...
- **CLAUDE.md / internal framing (this file)** = the seed-kernel recursion is the soul. The 4-tier Bayesian gate (T1 lexical → T2 embedding → T3 chairman → T4 Beta-Binomial) closes four feedback edges: T3 reads lens tensions AS its rubric (not background context), T3↔T4 calibration tunes per-basin baselines from observed demotion rate, the gate-over-lens primitive applies T1/T2 to lens tensions themselves, and the chairman that trains the lens is the same model the lens grades. The kernel applying to its own substrate. Code reference: `src/trinity_local/moves/gate.py` + `moves/dream.py` Phase 6d. Recursion isn't the moat — it's the reason the moat is defensible.
