...
> - [`docs/three-tier-architecture.md`](docs/three-tier-architecture.md) — **launch architecture**, ratified by `council_ff3da1fa84906791` (2026-05-16). Three tiers — Skill (primary), Pip (engine), Chrome Extension (optional capture+UI) — with `~/.trinity/` as the invariant data contract. v1.0 ships the skill artifact at `skills/trinity/SKILL.md` orchestrating the existing CLI; the shared `scripts/` substrate + cross-backend equivalence test harness defer to v1.1. Tier-equivalence invariant (NOT bit-identical): cosine ≥ 0.9999 between backends, identical k-means cluster assignments, identical chairman picker output under pinned config.
...
- `pytest -q` — **1242 passed** (32 in `test_doc_count_consistency.py` defending launch-credibility claims).
