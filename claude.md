...
Shortcut dispatcher, watcher subsystem, embedding cache, the
source-side `src/trinity_local/research/` package, 10+ internal
CLI commands — see CHANGELOG v1.7.4 entry). The user-state
`~/.trinity/research/` corpus directory is NOT retired — it's the
read-only hard-example corpus that knn_advisor still queries via
the live ranker chain.
...
2. **First-run wow demo (task #115, in_progress).** Cross-provider
   ...
   `trinity-local handoff antigravity` continues the thread on Gemini with
...
1. **MCP-dropdown distribution** (task #114). Get Trinity into the
   ...
   technically perfect. `install-mcp` already wires the four CLI
   harnesses (Claude Code → `~/.claude.json`, Antigravity →
   `~/.gemini/settings.json`, Codex → `~/.codex/config.toml`, Cursor
   → `~/.cursor/mcp.json`); the missing surface is *discoverability*
   — users who don't know Trinity exists never run `install-mcp`.
...
2. **First-run wow — cross-provider continuity, NOT council depth**
   ...
   `handoff antigravity` (or agent suggests it). Gemini picks up exactly
...
4. **Standardize `~/.trinity/`** (task #117 — ✓ shipped). If Aider,
   Cline, Continue adopt the same preference-corpus schema, Trinity
   becomes a standard, not a product. Standards have ~10× the
   longevity of products. JSON Schemas for `council_outcomes/*.json`,
   `eval_set`, `rejection_signal`, and `trust` ship in
   `skills/trinity/schemas/`. Adoption outreach to Aider / Cline /
   Continue is the remaining work (publishing was the shippable
   half; adoption is downstream).

5. **Subsidy-window narrative** (task #118 — ✓ shipped). Tell users
   explicitly: "Programmatic credits are subsidized right now. Build
   your preference corpus while it's cheap; the corpus has lifetime
   value once subscriptions tighten." Legitimate FOMO motivator
   because it's true. Now threaded through launch copy, README hero,
   the onboarding ribbon, and the launch-day `docs/launch.md` tweet
   thread.
...
Trinity is exposed as an MCP server. Ships 4 canonical tools (`route`, `run_council`, `get_persona`, `get_council_status`); v1.5 adds `ask` (cheap default single-call routing), `get_picks` (agent-facing introspection into extracted picks), and `mark_pick_wrong` (user-veto on a pick — halves effective trust per click, persists across consolidations); launch-arc adds `handoff` (cross-provider continuity) — <!-- canonical:mcp_tool_count -->8<!-- /canonical --> total. (`record_outcome` retired 2026-05-21 per "we are sunsetting user ratings"; the chairman's `winner` IS the supervision signal now, fed into `compute_personal_routing_table()` automatically. CLI `council-rate` still works for power users.) `run_council(responses=[...])` covers what `judge` used to do — pre-supplied member outputs go straight to chairman synthesis, one model call instead of N+1. The chairman reads `~/.trinity/memories/lens.md` and condenses members through *this user's* taste — that's what makes the council more useful than just asking Claude alone.
...
After a council, treat the chairman's synthesis (especially `agreed_claims` / `disagreed_claims`) as the source of truth for what the answer should be, then explain it in your own words back to the user. The chairman's pick is already recorded in `~/.trinity/council_outcomes/<id>.json` and will feed the personal routing table automatically — no rating call needed (record_outcome retired 2026-05-21).
...
Run `trinity-local install-mcp` once to register Trinity's MCP server with Claude Code (`~/.claude.json`), Antigravity (`~/.gemini/settings.json` — merges into existing `mcpServers`; both `agy` binaries read the same file), Codex CLI (`~/.codex/config.toml`), and Cursor (`~/.cursor/mcp.json` — same JSON shape as Claude Code; added per the P16/P92 persona audit when a Cursor user found Trinity was already MCP-compatible but no install path existed). Each harness spawns `trinity-local --mcp` as a stdio child when it starts; it lives until the harness exits. ~62MB resident while connected.
...
| Council schema | `council_schema.py` | `PromptBundle`, `LaunchEvent`, `CouncilMemberResult`, `CouncilChainStep`, `CouncilRoutingLabel` (with `agreed_claims` / `disagreed_claims`), `CouncilOutcome` (with `mode` and `chain_steps`) |
...
| Ranker | `ranker/` — `base.py`, `fallback.py`, `heuristic.py`, `knn_ranker.py`, `chairman_picker.py`, `types.py` | Routing decisions + chairman auto-selection (personal table → global benchmarks → default order) |
...
| MCP | `mcp_server.py` | canonical 4 + v1.5 `ask` + `get_picks` + `mark_pick_wrong` + launch-arc `handoff` (see below; `record_outcome` retired 2026-05-21) |
...
The full public surface is **<!-- canonical:mcp_tool_count -->8<!-- /canonical --> tools** — 4 canonical + 3 v1.5
additions + 1 launch-arc addition (`handoff`, tick #119, 2026-05-14).
The canonical 4 are the lifecycle order; the v1.5 trio sits adjacent
...
`ask` + `get_picks`. `record_outcome` retired 2026-05-21 — chairman's
pick is the supervision signal now.)

**Canonical four (lifecycle order — note: this is the *teaching* order, "what you call when, and why"; mcp_server.py registers tools in a UX order that interleaves the v1.5 trio for `tools/list` discoverability — cheap+common first: `route`, `ask`, then `run_council`, etc. Both orderings are correct for their purpose):**

1. **`route(task, harness?, available_models?, budget?, latency?, current_provider?)`** → `{mode, primary, challenger, confidence, reason, fallback}`. `task` is required; everything else is optional with sensible defaults. `budget` ∈ {`low`, `normal`, `high`} (default `normal`); `latency` ∈ {`fast`, `normal`, `patient`} (default `normal`). No model calls — heuristic + k-NN + chairman picker. Cheap, called before the harness picks a model.

2. **`run_council(task, goal?, members?, mode?, sequence?, primary_provider?, responses?, wait_seconds?)`** → council launched asynchronously. `task` is required; `goal` defaults to `"Find the strongest answer."` `mode="parallel"` (default) runs members concurrently then chairman. `mode="chain"` runs sequence serially with each step seeing prior outputs. **When `responses=[...]` is provided** (pre-supplied member outputs), skips dispatch and runs chairman synthesis only — one model call instead of N+1, returns the structured Routing JSON inline. This subsumes the former `judge` tool. `wait_seconds > 0` blocks up to that many seconds for the outcome inline; otherwise returns the `council_run_id` and the caller polls `get_council_status`.

3. **`get_persona()`** → returns `~/.trinity/memories/lens.md`. The chairman already loads this internally, but exposing it lets *any* harness (Claude Code, Codex, Antigravity) pull the persona once at session start and tailor responses without an MCP round trip per call.

4. **`get_council_status(council_run_id)`** → in-protocol polling for async councils. Returns status (running/completed/failed/canceled), per-member progress, synthesis state, and outcome summary (winner, agreed/disagreed claims, routing_lesson) when complete. Required for harnesses without filesystem access; also the only way to detect a stuck member without watching `~/.trinity/portal_pages/status/`.

(`record_outcome` retired 2026-05-21 — was tool #3 in this list. The chairman's `routing_label.winner` is the supervision signal now, computed into the personal routing table automatically. See "Why retired" in retired_names.py.)

**v1.5 trio:**

5. **`ask(query, available_providers?, top_k?)`** → cheap single-call default routing. The 90% case: harness has a question, wants one model's answer with a chairman-blessed verdict, doesn't need a full council. `query` is required; `available_providers` lets the caller restrict the dispatch lineup; `top_k` (default 5) controls how many past prompts feed the k-NN vote. Pulls from cortex picks first (high-trust rule → use directly), falls back to k-NN advisory, finally to heuristic.

6. **`get_picks(basin_id?, min_trust?)`** → agent-facing introspection into extracted cortex routing patterns. Returns `{rules: {basin_id: pattern}, total_basins, returned}` — patterns are keyed by basin_id (cortex consolidation is per-basin, not per-task_type), each carrying provider/reasoning/trust score/source councils inside the pattern dict. `basin_id` filter narrows to one basin; `min_trust` floor filters by `trust_score.value`. Empty cortex returns `{rules: {}, note: "..."}`. Lets a harness inspect what Trinity has learned about which model wins for which basin without firing route().

7. **`mark_pick_wrong(basin_id, reason?, reset?)`** → user-veto on an extracted pick. Required arg is `basin_id` (the basin / task_type whose cortex rule should be marked wrong). Each call increments `override_count`; `effective_trust = raw_trust × 0.5^count` — two clicks quarter the trust; three drops most rules out of routing entirely. Persists across consolidations — a fresh extraction can't erase the user's signal. `reset=true` clears the count (user changed their mind). Same shape as the launchpad's pick-veto chip (Surface 17) but from the agent side.

**Launch-arc addition (tick #119, 2026-05-14):**

8. **`handoff(target_provider, continuation?, num_turns?)`** → cross-provider conversation continuity. Pulls the user's most-recent (user, assistant) turns from the cross-provider prompt index, packages them as "continuing this thread" context, dispatches to a DIFFERENT provider. Target picks up exactly where the prior model left off — no re-context, no copy-paste. This is the mechanism behind the 60-second hero demo (#115/#120). Structurally non-refutable: only Trinity has the cross-provider index. CLI mirror: `trinity-local handoff <provider>`.
...
│   └── dispatch_outcomes.jsonl     #   `ask` + member dispatch outcomes — fuels `dispatch_health.py` (per-provider 24h health window: success_rate, rate_limit_count, last_error_class)
│
│  ── First-run state ────────────────────────────────────────────
├── cold_start_scan.json            # First-spawn auto-scan (status, sources, added count)
│
│  ── Optional user-curated corpus (read-only for Trinity) ───────
└── research/                       # k-NN advisory corpus (see knn_advisor.py)
    ├── hard_examples/              #   curated tough routing decisions
    └── examples/                   #   replay-style examples
```

(The user-curated `~/.trinity/research/` corpus IS still live —
`knn_advisor.advise()` reads it through `state_paths.research_dir()`
to upgrade the heuristic ranker's decisions in `ranker/knn_ranker.py`.
Empty corpus → advisor returns None → ranker falls through to
heuristic. The directory's sub-helpers `hard_examples_dir()` and
`replay_examples_dir()` ARE retired — those mkdir'd unused dirs and
had zero callers; knn_advisor inlines the path strings via
`research_dir() / "hard_examples"` instead. What got retired in the
pre-launch simplification pass was the source-side
`src/trinity_local/research/` PACKAGE, not the user-state directory.)
...
(`~/.trinity/research/` is NOT retired — see the "Optional user-
curated corpus" note above the retired-list — only the source-side
...
PromptNode → TurnWindow (per-prompt + per-window context)
    ↓ memory.search_prompt_nodes (substring × 2.5 + replay-value heuristics; no embeddings on hot path per task #54)
...
Every council outputs one labeled training example for the eventual Phase 9 learned router. Removing any of `route` or `run_council` breaks a meaningful surface. (`record_outcome` was the third load-bearing tool but was retired 2026-05-21; the chairman's pick is now the supervision signal, fed automatically.)
...
- **Minimal runtime dependencies.** Three: `Pillow>=10` (PNG share cards), `mcp>=1.0` (MCP server runtime — `install-mcp` registers Trinity in Claude Code / Codex / Antigravity / Cursor), `numpy>=1.26` (lens-build cosine matmul in `me_builder.py:138`, k-means in `me/basins.py`, vocabulary stats — NOT search; `search_prompt_nodes` is pure heuristic since task #54). `[mlx]` extras add `sentence-transformers>=2.2`, `einops`, and `torch>=2.0` for real 768d embeddings; without them the embedder falls back to the stable SHA-1 TF-IDF projection. `[test]` extras add `pytest>=7`.
...
1. **Memory index live.** `seed-from-taste-terminal` populates `~/.trinity/prompts/` from claude_ai + chatgpt + gemini takeout exports. 768d nomic embeddings written per PromptNode, batched. Search (`search_prompt_nodes`) is pure heuristic since task #54 — substring × 2.5 + replay-value signals, no embedding model load on the hot path. Embeddings stay live for the lens-build pipeline (Stage 1 k-means + Stage 4 cosine similarity in `me_builder.py:138`), the dream Phase 1 cross-provider pair discovery, and cortex consolidation. The 49k-vector ~5ms search number on real corpora reflects the heuristic path, not the (now retired-for-search) numpy matmul fast-path.
...
6. **MCP tool surface (canonical 4 + v1.5 `ask` + `get_picks` + `mark_pick_wrong` + launch-arc `handoff`).** Canonical: `route`, `run_council` (subsumes `judge` via `responses=[...]`), `get_persona`, `get_council_status`. v1.5 adds `ask` (cheap default single-call routing — the 90% case), `get_picks` (agent-facing introspection into extracted picks), and `mark_pick_wrong` (user-veto on a pick; halves effective trust per click) — 7 total before launch-arc. Launch-arc adds `handoff` — 8 live tools. (`record_outcome` retired 2026-05-21 alongside rest of rating UX — chairman's pick is now the supervision signal. `get_eval_summary` retired 2026-05-18 in commit `1fed7fc` — agents ground via `ask` + picks.) Legacy tools dropped from public surface: get_status / get_elo / get_recent_councils / watch_once / judge / record_outcome.
...
a future `plan_and_execute` MCP tool — task #128, still pending; the original v1.7 target slipped when v1.7's actual scope became MCP-primary + post-launch consistency sweep, and v1.6 turned out to be browser-extension capture — driven by a flagship Conductor
...
- `pytest -q` — **<!-- canonical:test_count -->1653<!-- /canonical --> passed** + <!-- canonical:skipped_count -->4<!-- /canonical --> skipped (<!-- canonical:doc_consistency_guards -->102<!-- /canonical --> in `test_doc_count_consistency.py` defending launch-credibility claims; 4 skipped are the gated real-Chrome smokes that need a loaded extension to run).
- `trinity-local --mcp` exposes <!-- canonical:mcp_tool_count -->8<!-- /canonical --> tools: the canonical 4 (`route`, `run_council`, `get_persona`, `get_council_status`) + v1.5 `ask` (cheap single-call routing) + v1.5 `get_picks` (agent-facing introspection into extracted picks) + v1.5 `mark_pick_wrong` (user-veto on a pick; halves effective trust per click) + launch-arc `handoff` (cross-provider conversation continuity). (`record_outcome` retired 2026-05-21 — chairman's pick is the supervision signal.)
