...
12. **Test suite: 1296 passing** + 4 skipped (post-Pass-A-BB simplification: ~80 tests dropped alongside the watcher subsystem, distill CLI, metric/stats CLIs, council-last, depth-show, trust CLI deferral, embedding cache, and other surface kills. Net change from launch baseline 1372 → simplification 1293 → consistency sweep through iter #62 added 3 net new tests = 1296. The four skipped tests are gated real-Chrome smokes that need a loaded extension to run).
...
- `pytest -q` — **1296 passed** + 4 skipped (41 in `test_doc_count_consistency.py` defending launch-credibility claims; 4 skipped are the gated real-Chrome smokes that need a loaded extension to run).
