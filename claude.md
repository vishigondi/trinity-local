...
CLI dispatcher: `src/trinity_local/main.py` (thin; `set_defaults(handler=...)`). 23 user-facing command modules in `commands/`. Notable: `dream.py` (cold-start verb), `me.py` (`lens-build`), `council.py` (`council-start` / `council-launch` / `council-stop` / `council-share` / `council-iterate`), `cortex.py` (`consolidate` + `cortex-override`), `eval.py` (`eval-build` / `eval-run` / `eval-show` / `eval-share`), `extension_repair.py` (`extension repair` — diagnose capture failures and dispatch a council to propose a page-hook.js patch; the "Trinity heals itself" flagship demo), `decision_log.py` (`decision-log` — capture strategic trade-offs at decision-time with `would_flip_if` counterfactual; HIGH-QUALITY signal weight=2.0 to Stage 2), `handoff.py`, `install.py` (`install-mcp` writes Claude Code / Codex / Antigravity / Cursor configs — 4 CLI harnesses), plus `adapters.py`, `debug.py`, `download_embedder.py`, `install_umbrella.py`, `me_card.py`, `portal.py`, `replay.py`, `review.py`, `seed.py`, `status.py`, `telemetry.py`, `update.py`, `vocabulary.py`, `watch.py` (`ingest-recent`).
