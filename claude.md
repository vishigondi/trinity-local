...
Trinity is an MCP-first local memory layer that reads existing transcripts (Claude Code, Codex, Antigravity, Cursor, plus claude.ai / chatgpt.com / Gemini captures and exports), extracts a **lens** from how the user rephrases, judges, and decides, then runs hard questions through multiple providers and synthesizes the answer the user would have picked.

**Current state (2026-05-30):** `v<!-- canonical:version -->1.7.110<!-- /canonical -->` is the live repo state. The advertised CLI surface is `trinity-local lens`, `trinity-local council`, `dream`, `status`, and `install`; older names such as `lens-build` and `council-launch` remain compatibility aliases for launchpad dispatch, extension allowlists, and scripts. The recent arc hardened launch readiness: Chrome extension auto-wire, schema migrations, real ModernBERT embeddings, TF-IDF abstain-gates for semantic flows, corpus-purity and noise filters, trajectory + correction-vector lenses, personal eval integrity, no-PII telemetry gates, and install-wrapper Python fallback. The full timestamped history lives in [CHANGELOG.md](CHANGELOG.md).

**Verification snapshot:** <!-- canonical:test_count -->2447<!-- /canonical --> tests passing + <!-- canonical:skipped_count -->4<!-- /canonical --> skipped; <!-- canonical:doc_consistency_guards -->111<!-- /canonical --> doc-consistency guards; <!-- canonical:mcp_tool_count -->8<!-- /canonical --> MCP tools; <!-- canonical:cli_command_count -->49<!-- /canonical --> CLI subcommands across <!-- canonical:command_module_count -->21<!-- /canonical --> user-facing command modules; <!-- canonical:smoke_surface_count -->32<!-- /canonical -->-surface browser smoke gate.
...
5. **HF Hub offline by default.** `main()` pins `HF_HUB_OFFLINE=1` + `TRANSFORMERS_OFFLINE=1` via `setdefault` at startup. The embedding model is pulled once via `trinity-local download-embedder` (or `HF_HUB_OFFLINE=0 huggingface-cli download nomic-ai/modernbert-embed-base`); after that Trinity loads from `~/.cache/huggingface/hub/` and never contacts the Hub during normal operation.
...
Beyond tools/resources/sampling, Trinity exposes the full MCP capability set (v1.7.96, #264): **Prompts** (`council` / `ask` / `lens` as native slash-menu entries via `list_prompts`/`get_prompt`), **Logging** (`set_logging_level` + `mcp_features.mcp_log`, emitted at council launch/complete), **Progress** (`mcp_progress` streams a live bar during a blocking `run_council` wait), **Roots** (`discover_roots` reads the client's exposed filesystem roots on first tool call), and **Elicitation** (`mcp_features.elicit` — structured mid-tool prompts, primitive-ready, degrades to None when the harness doesn't support it). All helpers live in `mcp_features.py`, reuse the `mcp_sampling._active` session/loop (thread-safe via `run_coroutine_threadsafe`), and quietly no-op when no capable client is connected.

The same `mcp_sampling._active` session powers the **MCP-sampling lens build** keystone (#263, v1.7.95): the activity-gated first build + refresh fire from `_maybe_fire_lens_kicks` on the *first tool call* (not at startup — that's before any session is registered), and the worker thread inherits the session via `contextvars.copy_context()`, so Stage 0/2/3 + distill chairman calls route through `sampling/createMessage` on the user's live Claude Code session — zero `claude -p` quota. Falls back to the subprocess when no sampling-capable session is active (CLI use, Codex/Cursor).
...
│   └── vocabulary.md               #   anchors + homonyms (linguistic)
...
├── me/                             # Lens-build pipeline output (preference_acts/lenses/orderings/arcs/trajectories/lens_registry)
│
...
- **Graceful degradation** — features depending on `[mlx]` fall back; analytics never crash. But a fallback that emits *wrong* output is worse than none: post-#106/#109, semantic flows (correction/taste lens, cortex centroids, the geometric noise filter) **abstain** under the SHA-1 TF-IDF backend rather than ship inverted-TF-IDF garbage — they return `{ready: False, reason: "needs real embeddings"}` and the cold-open degrades to the always-correct lexical anchors.

## Launch arc

The five-workstream distribution arc and full forward-arc narrative now live in [`docs/historical/brand-evolution.md`](docs/historical/brand-evolution.md) and [`docs/historical/scale-plan.md`](docs/historical/scale-plan.md). Known-shipped workstreams: task #114 (MCP-dropdown distribution), #115 (First-run wow / 60s cross-provider continuity demo), #117 (Standardize `~/.trinity/`), #118 (Subsidy-window narrative), #122 (Corpus-based eval harness), #135 (gemini.google.com Chrome-capture adapter), #136 (`trinity-local extension repair` flagship demo), #162 (MCP Resources). The v2 moves substrate (#167–#172, #181) was retired 2026-05-27 in #184 after a real-data dream cycle proved the 4-tier gate structurally dormant — chairman LLM bridges declarative→procedural at inference time, making moves a redundant projection. Recently completed follow-ups include #182 trajectory lens, #183 schema versioning, and the #185-#189 guard/orphan-detection infrastructure. Active launch risks now sit around public-extension/Web-Store sequencing, native Windows beyond WSL2, and packaging/install honesty rather than core council mechanics. The `principles.md` pipeline remains data-gated (needs >=100 council outcomes). (Tasks #119 handoff + #142 memory-compare retired 2026-05-26 — see retired_names.py.)

## Verified status

- `pytest -q` — **<!-- canonical:test_count -->2447<!-- /canonical --> passed** + <!-- canonical:skipped_count -->4<!-- /canonical --> skipped (167s wall, all gated real-Chrome smokes intentionally skipped). <!-- canonical:doc_consistency_guards -->111<!-- /canonical --> doc-consistency guards in `test_doc_count_consistency.py` defending launch-credibility claims. The screenshot-freshness guard (`TestLaunchpadScreenshotFreshness::test_launchpad_example_not_grossly_stale`) is the canary — it fires when `docs/launchpad_example.png` falls more than a day behind `launchpad_template.py`; clear it with the regen recipe in the test's error message.
...
- [`docs/historical/principles.md`](docs/historical/principles.md) — 31 numbered meta-principles extracted from the fixes. Each earned its place by costing time.
