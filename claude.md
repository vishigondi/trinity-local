...
- **Minimal runtime dependencies.** Three: `Pillow>=10` (PNG share cards), `mcp>=1.0` (MCP server runtime — `install-mcp` registers Trinity in Claude Code / Codex / Antigravity), `numpy>=1.26` (embedding matmul fast-path + k-means + vocabulary stats). `[mlx]` extras add `sentence-transformers>=2.2`, `einops`, and `torch>=2.0` for real 768d embeddings; without them the embedder falls back to the stable SHA-1 TF-IDF projection. `[test]` extras add `pytest>=7`.
...
- `pytest -q` — **<!-- canonical:test_count -->1609<!-- /canonical --> passed** + <!-- canonical:skipped_count -->4<!-- /canonical --> skipped (<!-- canonical:doc_consistency_guards -->65<!-- /canonical --> in `test_doc_count_consistency.py` defending launch-credibility claims; 4 skipped are the gated real-Chrome smokes that need a loaded extension to run).
