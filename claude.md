...
The full public surface is **8 tools** — 4 canonical + 3 v1.5
...
- `pytest -q` — **<!-- canonical:test_count -->1612<!-- /canonical --> passed** + <!-- canonical:skipped_count -->4<!-- /canonical --> skipped (<!-- canonical:doc_consistency_guards -->68<!-- /canonical --> in `test_doc_count_consistency.py` defending launch-credibility claims; 4 skipped are the gated real-Chrome smokes that need a loaded extension to run).
- `trinity-local --mcp` exposes 8 tools: the canonical 4 (`route`, `run_council`, `get_persona`, `get_council_status`) + v1.5 `ask` (cheap single-call routing) + v1.5 `get_picks` (agent-facing introspection into extracted picks) + v1.5 `mark_pick_wrong` (user-veto on a pick; halves effective trust per click) + launch-arc `handoff` (cross-provider conversation continuity). (`record_outcome` retired 2026-05-21 — chairman's pick is the supervision signal.)
