...
| `commands/eval.py` | `eval-build` / `eval-stats` / `eval-run` (corpus-based eval harness — task #122; `eval-build` produces the suite from `me/rejections.jsonl`, `eval-run --target <provider>` dispatches each prompt + scores via judge against `lens.md`, results land in `~/.trinity/evals/results/`. The empirical benchmark — unblocks #116.) |
