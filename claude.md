...
12. **Test suite: 1384 passing** (1290 baseline at session start → 1382 after the three-tier restructure landed 82 new tests across scripts/, trust/audit, tier-equivalence, fresh-install, and Phase 7 council pre-empts; +2 from the May 17 public-readiness pass that fixed personal-path leaks in `test_frontend_flow.py`).
...
- `pytest -q` — **1384 passed** + 4 skipped (33 in `test_doc_count_consistency.py` defending launch-credibility claims; 4 skipped are the gated real-Chrome smokes that need a loaded extension to run).
