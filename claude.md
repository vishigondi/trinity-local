...
Entry: `src/trinity_local/main.py` — thin dispatcher only. Live CLI surface after pre-launch simplification (Passes A–BB collapsed task/bundle/launch/watch/distill/cache/depth/metric/trust/shortcut/council-last/auto-chain/auto-open). 22 user-facing command modules (21 in `CORE_COMMAND_MODULES` + `install` in `OPTIONAL_COMMAND_MODULES`); 7 more (`bootstrap_pairs`, `depth`, `distill`, `helpers`, `ingest`, `tasks`, `trust`) survive as importable utilities for tests + internal callers but no longer register CLIs (the `shortcuts_integration` inert shim at the package root falls into the same category). Live argparse surface: <!-- canonical:cli_command_count -->44<!-- /canonical --> subparser registrations (each one shows in `trinity-local --help`); count auto-rendered from main.py's registration via `scripts/render_docs.py` so docs and the actual CLI surface can't drift.
...
12. **Test suite: <!-- canonical:test_count -->1564<!-- /canonical --> passing** + <!-- canonical:skipped_count -->4<!-- /canonical --> skipped (post-Pass-A-BB simplification: ~80 tests dropped alongside the watcher subsystem, distill CLI, metric/stats CLIs, council-last, depth-show, trust CLI deferral, embedding cache, and other surface kills. Net change from launch baseline 1372 → simplification 1293 → consistency sweep through iter #62 added 3 net new tests = 1296. The four skipped tests are gated real-Chrome smokes that need a loaded extension to run).
...
- `pytest -q` — **<!-- canonical:test_count -->1564<!-- /canonical --> passed** + <!-- canonical:skipped_count -->4<!-- /canonical --> skipped (<!-- canonical:doc_consistency_guards -->44<!-- /canonical --> in `test_doc_count_consistency.py` defending launch-credibility claims; 4 skipped are the gated real-Chrome smokes that need a loaded extension to run).
