...
The full public surface is **<!-- canonical:mcp_tool_count -->9<!-- /canonical --> tools** — 4 canonical + 3 v1.5 additions + 1 launch-arc addition (`handoff`) + 1 in-protocol provider-side loop (`import_provider_memory`). Tool docstrings ARE the contract the agent reads at handshake.
...
CLI dispatcher: `src/trinity_local/main.py` (thin; `set_defaults(handler=...)`). 25 user-facing command modules in `commands/`. Notable: `dream.py` (cold-start verb), `me.py` (`lens-build`), `council.py` (`council-start` / `council-launch` / `council-stop` / `council-share` / `council-iterate`), `cortex.py` (`consolidate` + `cortex-override`), `eval.py` (`eval-build` / `eval-run` / `eval-show` / `eval-share`), `extension_repair.py` (`extension repair` — diagnose capture failures and dispatch a council to propose a page-hook.js patch; the "Trinity heals itself" flagship demo. `--auto` (#147) skips the HAR ask and auto-dispatches when a code-patch pattern surfaces; the launchpad's "Repair extension" button fires `--auto --json`.), `decision_log.py` (`decision-log` — capture strategic trade-offs at decision-time with `would_flip_if` counterfactual; HIGH-QUALITY signal weight=2.0 to Stage 2), `memory_compare.py` (`memory-compare` — side-by-side Trinity-lens vs Claude-Auto-Dream lexical comparison, task #142 Mode 1), `import_export.py` (`import-export` — auto-detect + bulk-import Takeout / web exports from any path, task #148), `handoff.py`, `install.py` (`install-mcp` writes Claude Code / Codex / Antigravity / Cursor configs — 4 CLI harnesses), plus `adapters.py`, `debug.py`, `download_embedder.py`, `install_umbrella.py`, `me_card.py`, `portal.py`, `replay.py`, `review.py`, `seed.py`, `status.py`, `telemetry.py`, `update.py`, `vocabulary.py`, `watch.py` (`ingest-recent`).
